import type { CampaignConfig, CardFace, CardRecord, CharacterAbility, CharacterConfig, CharacterDefinition, EffectType, FrequencyLevel, LearningStore, ReviewStore, EnemyDefinition } from '../types'
...
export async function loadCardLibrary(): Promise<CardRecord[]> {
  const response = await fetch('/data/cet6_cards.json')
...
export function isEnemyDefinition(value: unknown): value is EnemyDefinition {
  ...
  if (!['skull', 'flame', 'eye', 'crown', 'zap', 'shield'].includes(enemy.icon as string)) return false
  ...
  return enemy.abilities.every((ability) => {
    ...
    if (!['fixed-shield-per-turn', 'attack-scaling'].includes(item.type as string) || !Number.isInteger(item.amount) || (item.amount as number) < 0 || typeof item.description !== 'string' || item.description.trim().length === 0) return false
...
export function isCharacterDefinition(value: unknown): value is CharacterDefinition {
  ...
  if (!['skull', 'flame', 'eye', 'crown', 'zap', 'shield'].includes(character.icon as string)) return false
...
export function isCharacterAbility(value: unknown): value is CharacterAbility {
  ...
  const passiveTypes = ['passive-start-shield', 'passive-max-hp', 'passive-heal-per-turn']
...
export function isCharacterConfig(value: unknown): value is CharacterConfig {
  ...
  return config.characters.every((character) => isCharacterDefinition(character) && !ids.has(character.id) && Boolean(ids.add(character.id)))
}
...
export function makeRuntimeCards(cards: CardRecord[], random: () => number = Math.random, ensureSpelling = cards.length > 1): import('../types').RuntimeCard[] {
