import type { BattleMode, BattleState, CharacterAbility, CharacterDefinition, CharacterState, EnemyDefinition, EffectType, RuntimeCard } from '../types'
import { makeRuntimeCards } from './data'
...
export function effectDescription(card: RuntimeCard): string {
  ...
  switch (card.card.effectType) {
    case 'attack': return `造成 ${3 + level} 点伤害`
    case 'shield': return `获得 ${3 + level} 点护盾`
    case 'boost': return `下一张攻击或护盾 +${level}`
    case 'draw': return `额外抽 ${1 + Math.floor(level / 2)} 张牌`
    case 'heal': return `回复 ${2 + Math.ceil(level / 2)} 点生命`
  }
}

export function enemyAttack(state: BattleState): number {
  return state.enemy.attack + state.enemy.abilities
...
function enemyState(definition: EnemyDefinition) {
  return { ...definition, hp: definition.maxHp, shield: 0, turns: 0 }
}
...
export function createBattle(drawPile: import('../types').CardRecord[], hand: RuntimeCard[], mode: BattleMode = 'practice', definition: EnemyDefinition = DEFAULT_ENEMY, character: CharacterDefinition = DEFAULT_CHARACTER): BattleState {
...
export function applyCardEffect(state: BattleState, card: RuntimeCard): { state: BattleState; summary: string } {
  ...
  switch (card.card.effectType) {
    ...
    case 'heal': {
      const amount = 2 + Math.ceil(level / 2)
...
export function drawTurnCards(state: BattleState, cards: import('../types').CardRecord[]): { state: BattleState; overflow: number } {
  const next = structuredClone(state) as BattleState
...
export function finishEnemyTurn(state: BattleState): BattleState {
  ...
  next.log = [`${next.enemy.name} 攻击 ${incoming} 点，获得 ${shieldGain} 点护盾，己方护盾抵挡 ${blocked} 点，受到 ${damage} 点伤害。`, ...next.log].slice(0, 8)
  if (next.player.hp <= 0) next.status = 'defeat'
  return next
}
