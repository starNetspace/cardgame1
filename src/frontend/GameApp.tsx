...
import type { AccountRegistry, BattleMode, BattleState, BattleStore, CampaignConfig, CampaignRoute, CardMemoryAnswerSource, CardMemoryQuality, CardMemoryRecord, CardRecord, CardSource, CharacterConfig, EnemyDefinition, EnemyIcon, LearningStore, QuestionState, ReviewSession, ReviewStore, RuntimeCard, StudyDeck } from '../shared/domain-types'
import './app-styles.css'

type View = 'menu' | 'modes' | 'learning-decks' | 'campaign-setup' | 'character-select' | 'battle' | 'result' | 'stats' | 'library' | 'mistakes' | 'accounts'
type MistakeBookPage = 'choose' | 'all' | 'memory'
...
type GradeFilter = 'all' | CardMemoryQuality

export const GRADE_FILTER_OPTIONS: Array<{ key: GradeFilter; label: string }> = [
  { key: 'all', label: '全部' },
  { key: 'bronze', label: '青铜' },
  { key: 'silver', label: '白银' },
  { key: 'gold', label: '黄金' },
...
export function filterCardMemoryByGrade(records: CardMemoryRecord[], grade: GradeFilter): CardMemoryRecord[] {
  if (grade === 'all') return records
  return records.filter((record) => record.quality === grade)
}
...
function Portrait({ avatar, icon, size = 42 }: { avatar?: string; icon?: EnemyIcon; size?: number }) {
  ...
  return <img className="avatar-image" src={`/library/avatars/${encodeURIComponent(avatar)}`} alt="" loading="eager" onError={() => setImageFailed(true)} />
}
...
export default function App() {
  ...
  function startCampaign(start: CampaignStart, route: CampaignRoute, characterId: string) {
    ...
    const character = characterConfig.characters.find((item) => item.id === characterId) ?? characterConfig.characters.find((item) => item.id === characterConfig.selectedCharacterId) ?? characterConfig.characters[0]
    if (!character) return
  ...
  return <main className="app-shell">
    <header className="topbar">
      ...
      <div className="topbar-actions"><span className="library-count"><BookOpen size={15} /> {library.length.toLocaleString()} 张词卡</span><button className={`nav-button ${view === 'stats' ? 'active' : ''}`} onClick={() => setView('stats')}><BarChart3 size={16} />学习统计</button><button className={`nav-button ${view === 'accounts' ? 'active' : ''}`} onClick={openAccounts}><UserRound size={16} />{accountRegistry.activeUsername}</button></div>
    </header>
...
function CampaignSetup({ start, config, characters, defaultCharacterId, decks, onSelectCharacter, onStart, onBack }: { start: CampaignStart; config: CampaignConfig; characters: CharacterConfig; defaultCharacterId?: string; decks: StudyDeck[]; onSelectCharacter: () => void; onStart: (route: CampaignRoute, characterId: string) => void; onBack: () => void }) {
  const characterId = start.characterId ?? defaultCharacterId ?? characters.selectedCharacterId ?? characters.characters[0]?.id ?? ''
...
function CharacterSelect({ characterConfig, selectedCharacterId, onSelect, onBack }: { characterConfig: CharacterConfig; selectedCharacterId?: string; onSelect: (characterId: string) => void; onBack: () => void }) {
  ...
  const [previewId, setPreviewId] = useState(selectedCharacterId ?? characterConfig.characters[0]?.id ?? '')
...
function MistakeBook({ review, learning, cardMemory, library, initialPage, onReview, onPractice, onLearning, onAll, onBack }: { review: ReviewStore; learning: LearningStore; cardMemory: ReturnType<typeof loadCardMemoryStore>; library: CardRecord[]; initialPage: MistakeBookPage; onReview: () => void; onPractice: (maxCount?: number) => void; onLearning: (maxCount?: number) => void; onAll: (maxCount?: number) => void; onBack: () => void }) {
  ...
  const [currentGradeFilter, setCurrentGradeFilter] = useState<GradeFilter>('all')
  ...
  const memoryRecords = Object.values(cardMemory)
  const visibleRecords = filterCardMemoryByGrade(memoryRecords, currentGradeFilter)
  const cardFor = (id: string) => library.find((card) => card.cardId === id)
  const mistakeWordIds = [...new Set([...practiceIds, ...studyIds])]
  const renderWords = (ids: string[]) => <div className="mistake-book-list">{ids.map((id) => { const card = cardFor(id); return card ? <div className="mistake-book-row" key={id}><div className="mistake-book-word"><strong>{card.word}</strong><span className={`library-face-tag face-${card.face}`}>{card.face === 'meaning' ? '识义' : '拼写'}</span></div><span>{card.pos}</span><em>{card.meaning}</em><button className="speech-button mistake-speech-button" aria-label={`播放 ${card.word} 的发音`} onClick={() => speakWord(card.word, card.pos)}><Volume2 size={15} /></button></div> : null })}</div>
  const renderMemoryRecords = () => <div className="memory-record-list">{visibleRecords.map((record) => { const card = cardFor(record.cardId); return <article className={`memory-record quality-${record.quality}`} key={record.cardId}><div className="memory-record-main"><div><strong>{card?.word ?? record.cardId}</strong><span className={`quality-badge quality-${record.quality}`}>{({ bronze: '青铜', silver: '白银', gold: '黄金', mastered: '已掌握' } as const)[record.quality]}</span><small>{card ? `${card.pos} · ${card.meaning}` : '词库中暂无此卡片'}</small></div>{card && <button className="speech-button mistake-speech-button" aria-label={`播放 ${card.word} 的发音`} onClick={() => speakWord(card.word, card.pos)}><Volume2 size={15} /></button>}</div><div className="memory-record-meta"><span>连对 {record.streak}</span><span>错误 {record.lapses}</span><span className={record.dueAt <= Date.now() ? 'is-due' : ''}>{record.dueAt <= Date.now() ? '已到期' : `下次：${new Date(record.dueAt).toLocaleDateString()}`}</span></div></article> })}{memoryRecords.length === 0 && <div className="stats-empty"><Check size={18} />暂无复习卡片，答错的单词会出现在这里。</div>}{memoryRecords.length > 0 && visibleRecords.length === 0 && <div className="stats-empty"><Check size={18} />该分级下暂无卡片。</div>}</div>
  return <section className="mistake-book-page"><button className="back-link" onClick={onBack}>← 返回首页</button><div className="mistake-book-heading"><div><div className="eyebrow"><span className="eyebrow-dot" /> MISTAKE BOOK</div><h1>错题本</h1><p>把答错的词重新带回战场，连续答对三次后自动移除。</p></div><div className="mistake-book-review"><button className="button primary" disabled={memorySummary.due === 0} onClick={onReview}><RotateCcw size={15} />开始复习（{memorySummary.due}）</button><small>本局取最早到期的 20 张</small></div><div className="mistake-book-total"><strong>{allCount}</strong><span>个待复习词</span></div></div><div className="mistake-book-tabs"><button className={page === 'choose' ? 'active' : ''} onClick={() => setPage('choose')}>选择练习</button><button className={page === 'all' ? 'active' : ''} onClick={() => setPage('all')}>查看所有错题</button><button className={page === 'memory' ? 'active' : ''} onClick={() => setPage('memory')}>记忆档案 {memorySummary.total}</button></div>{page === 'memory' && <div className="grade-filter-bar" role="tablist" aria-label="分级筛选">{GRADE_FILTER_OPTIONS.map((option) => <button key={option.key} type="button" className={`grade-filter-button grade-${option.key}${currentGradeFilter === option.key ? ' active' : ''}`} aria-selected={currentGradeFilter === option.key} onClick={() => setCurrentGradeFilter(option.key)}>{option.label}</button>)}</div>}{page === 'choose' ? <><div className="mistake-limit-control"><label htmlFor="mistake-limit">本次最多练习</label><input id="mistake-limit" type="number" min="10" step="1" inputMode="numeric" value={maxCount} onChange={(event) => setMaxCount(event.target.value)} placeholder="全部" /><span>个错词，最少 10 个</span></div>{requestedCount !== undefined && !Number.isInteger(requestedCount) || requestedCount !== undefined && requestedCount < 10 ? <p className="mistake-limit-error">请输入不小于 10 的整数。</p> : null}<div className="mistake-choice-grid"><article><div className="choice-icon"><Crosshair size={22} /></div><strong>练习模式错题</strong><p>{practiceIds.size} 个词 · 使用练习战斗规则</p><button className="button primary" disabled={!validPractice} onClick={() => onPractice(practiceLimit)}><Swords size={15} />开始练习</button></article><article><div className="choice-icon"><BookOpen size={22} /></div><strong>学习模式错题</strong><p>{studyIds.size} 个词 · 按学习记录追踪</p><button className="button primary" disabled={!validLearning} onClick={() => onLearning(practiceLimit)}><BookOpen size={15} />开始学习错题</button></article><article><div className="choice-icon"><Target size={22} /></div><strong>全部错题</strong><p>{allCount} 个词 · 合并两类错题</p><button className="button ghost" disabled={!validAll} onClick={() => onAll(practiceLimit)}><RotateCcw size={15} />混合练习</button></article></div></> : page === 'all' ? <><div className="mistake-book-tabs-sub"><span>练习与学习错词</span><strong>{mistakeWordIds.length}</strong></div>{mistakeWordIds.length === 0 ? <div className="stats-empty"><Check size={18} />还没有错题记录。</div> : renderWords(mistakeWordIds)}</> : <><div className="memory-summary"><span>总档案 <strong>{memorySummary.total}</strong></span><span>已到期 <strong>{memorySummary.due}</strong></span><span>累计错误 <strong>{memorySummary.lapses}</strong></span></div>{memorySummary.total > 0 && memorySummary.due === 0 && <p className="review-next-due">下次到期：{nextDueAt ? new Date(nextDueAt).toLocaleString() : '—'}</p>}{renderMemoryRecords()}</>}</section>
}
...
function LibraryPage({ library, review, onBack }: { library: CardRecord[]; review: ReviewStore; onBack: () => void }) {
  const [query, setQuery] = useState('')
  const [field, setField] = useState<'word' | 'meaning' | 'all'>('all')
  const [firstLetter, setFirstLetter] = useState('')
  const [posQuery, setPosQuery] = useState('')
  const [level, setLevel] = useState<number | 'all'>('all')
  const [visibleCount, setVisibleCount] = useState(80)
  const normalizedQuery = query.trim().toLowerCase()
  const normalizedLetter = firstLetter.trim().slice(0, 1).toLowerCase()
  const normalizedPos = posQuery.trim().toLowerCase()
  const filtered = useMemo(() => library.filter((card) => {
    const word = card.word.trim().toLowerCase()
    const meaning = card.meaning.toLowerCase()
    const matchesLetter = !normalizedLetter || word.startsWith(normalizedLetter)
    const matchesPos = !normalizedPos || card.pos.toLowerCase().startsWith(normalizedPos)
    const matchesQuery = !normalizedQuery || (field === 'word' ? word.includes(normalizedQuery) : field === 'meaning' ? meaning.includes(normalizedQuery) : word.includes(normalizedQuery) || meaning.includes(normalizedQuery))
    return matchesLetter && matchesPos && matchesQuery && (level === 'all' || card.frequencyLevel === level)
  }), [library, normalizedQuery, normalizedLetter, normalizedPos, field, level])
...
return <section className="library-page"><div className="library-heading"><div><button className="back-link" onClick={onBack}><ChevronRight size={15} className="back-arrow" />返回首页</button><div className="eyebrow"><span className="eyebrow-dot" /> CET-6 WORD LIBRARY</div><h1>全部单词</h1><p>按英文、中文释义、首字母或词性检索你的词汇牌库。</p></div><div className="library-total"><strong>{library.length.toLocaleString()}</strong><span>张词卡</span></div></div><div className="library-toolbar"><label className="library-search"><Search size={17} /><input value={query} onChange={(event) => { setQuery(event.target.value); resetPage() }} placeholder="输入搜索关键词" aria-label="搜索关键词" />{query && <button type="button" onClick={() => { setQuery(''); resetPage() }} aria-label="清除搜索"><X size={15} /></button>}</label><div className="library-filter" role="group" aria-label="搜索范围"><span>范围</span><button className={field === 'word' ? 'active' : ''} onClick={() => { setField('word'); resetPage() }}>单词</button><button className={field === 'meaning' ? 'active' : ''} onClick={() => { setField('meaning'); resetPage() }}>中文释义</button><button className={field === 'all' ? 'active' : ''} onClick={() => { setField('all'); resetPage() }}>全部</button></div><label className="library-field"><span>首字母</span><input value={firstLetter} onChange={(event) => { setFirstLetter(event.target.value.replace(/[^a-zA-Z]/g, '').slice(0, 1).toUpperCase()); resetPage() }} aria-label="首字母" /></label><label className="library-field"><span>词性</span><input value={posQuery} onChange={(event) => { setPosQuery(event.target.value); resetPage() }} aria-label="词性" /></label><div className="library-filter" role="group" aria-label="筛选频率等级"><span>频率</span><button className={level === 'all' ? 'active' : ''} onClick={() => changeFilter('all')}>全部</button>{[1, 2, 3, 4, 5].map((item) => <button key={item} className={level === item ? 'active' : ''} onClick={() => changeFilter(item)}>L{item}</button>)}</div></div><div className="library-summary">显示 {visibleCards.length.toLocaleString()} / {filtered.length.toLocaleString()} 张词卡</div><div className="library-list">{visibleCards.map((card) => { const record = review[card.cardId]; return <article className="library-row" key={card.cardId}><div className="library-word"><div className="library-word-line"><strong>{card.word}</strong><button className="speech-button library-speech-button" aria-label={`播放 ${card.word} 的发音`} onClick={() => speakWord(card.word, card.pos)}><Volume2 size={15} /></button></div><small>{card.phonetic || '/ pronunciation /'}</small></div><div className="library-pos">{card.pos}</div><div className="library-meaning">{card.meaning}</div><div className="library-meta"><span className={`library-face-tag face-${card.face}`}>{card.face === 'meaning' ? '识义' : '拼写'}</span><span className={`level-dot level-${card.frequencyLevel}`}>L{card.frequencyLevel}</span><small>{record ? `${record.correct}/${record.attempts} 正确` : '未复习'}</small></div></article> })}{visibleCards.length === 0 && <div className="library-empty"><BookOpen size={20} />没有找到符合条件的词卡。</div>}</div>{visibleCards.length < filtered.length && <button className="button ghost library-load-more" onClick={() => setVisibleCount((count) => count + 80)}>加载更多<ChevronRight size={16} /></button>}</section>
}
