import type { AccountExport, AccountInfo, AccountRegistry, BattleStore, CardMemoryStore, LearningStore, ReviewStore } from '../shared/domain-types'
import { getActiveUsername, loadBattleStore, loadCardMemoryStore, loadLearningStore, loadReviewStore, normalizeUsername, saveBattleStore, saveCardMemoryStore, saveLearningStore, saveReviewStore, setActiveUsername } from './local-progress'
...
function isAccountInfo(value: unknown, username: string): value is AccountInfo {
  ...
  return info.username === username && Number.isFinite(info.createdAt) && Number.isFinite(info.lastUsedAt) && (info.selectedCharacterId === undefined || typeof info.selectedCharacterId === 'string')
}
...
export function saveSelectedCharacter(registry: AccountRegistry, characterId: string): AccountRegistry {
  const username = registry.activeUsername
  const account = registry.accounts[username]
  if (!account || !characterId.trim()) return registry
  const next = {
    ...registry,
    accounts: {
      ...registry.accounts,
      [username]: { ...account, selectedCharacterId: characterId },
    },
  }
  saveAccountRegistry(next)
  return next
...
export function createAccountExport(username = getActiveUsername(), exportedAt = Date.now()): AccountExport {
  ...
  const selectedCharacterId = loadAccountRegistry().accounts[normalizeUsername(username)]?.selectedCharacterId
  return { format: 'lexicon-duel-account', version: 1, username: normalizeUsername(username), exportedAt, ...(selectedCharacterId ? { selectedCharacterId } : {}), ...data }
}
...
export function parseAccountExport(value: unknown): AccountExport | null {
  ...
  if (data.selectedCharacterId !== undefined && (typeof data.selectedCharacterId !== 'string' || !data.selectedCharacterId.trim())) return null
...
export function importAccountData(registry: AccountRegistry, data: AccountExport, overwrite = true, now = Date.now()): AccountRegistry | null {
  ...
  const nextRegistry = registry.accounts[username]
    ? { ...registry, activeUsername: username, accounts: { ...registry.accounts, [username]: { ...registry.accounts[username], ...(data.selectedCharacterId ? { selectedCharacterId: data.selectedCharacterId } : {}), lastUsedAt: now } } }
    : { ...registry, activeUsername: username, accounts: { ...registry.accounts, [username]: { username, createdAt: now, lastUsedAt: now, ...(data.selectedCharacterId ? { selectedCharacterId: data.selectedCharacterId } : {}) } } }
