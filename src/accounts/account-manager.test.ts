...
import { createAccount, createAccountExport, deleteAccount, importAccountData, loadAccountRegistry, parseAccountExport, saveSelectedCharacter, switchAccount } from './account-manager'
import { createBattle } from '../battle/battle-rules'
import { getActiveUsername, loadBattleStore, loadLearningStore, loadReviewStore, saveBattleState, saveLearningStore, saveReviewStore, setActiveUsername, updateLearningMemory, updateReview } from './local-progress'
import type { AccountExport, LearningStore } from '../shared/domain-types'
...
describe('account management and portable data', () => {
  ...
  it('remembers a character choice per account and preserves it through export and import', () => {
    let registry = loadAccountRegistry()
    registry = createAccount(registry, 'Alice', 100)!
    registry = saveSelectedCharacter(registry, 'hero-alice')
    expect(registry.accounts.Alice.selectedCharacterId).toBe('hero-alice')

    registry = createAccount(registry, 'Bob', 200)!
    expect(registry.accounts.Bob.selectedCharacterId).toBeUndefined()
    registry = switchAccount(registry, 'Alice', 300)!
    expect(registry.accounts.Alice.selectedCharacterId).toBe('hero-alice')

    const exported = createAccountExport('Alice', 400)
    const parsed = parseAccountExport(JSON.parse(JSON.stringify(exported)))!
    registry = switchAccount(registry, 'Bob', 500)!
    const imported = importAccountData(registry, parsed, true, 600)!
    expect(imported.accounts.Alice.selectedCharacterId).toBe('hero-alice')
  })
